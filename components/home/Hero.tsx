...
import { ArrowUpFromDot, Sparkles } from "lucide-react";
...
export default function HeroSection() {
  ...
  const suggestions: string[] = ["Trending reels this week", "Gen Z gaming", "IPL 2025 sentiment"];
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const icons: string[] = [".float-yt", ".float-ig", ".float-tw", ".float-tt"];
  ...
  return (
    <div
      ref={heroRef}
      className="relative min-h-screen pt-[20vh] overflow-hidden flex flex-col items-center justify-center px-6 py-20"
    >
      {/* Background Blobs */}
      ...
      <div className="float-yt absolute left-[10%] top-[25%] drop-shadow-2xl drop-shadow-black/30 z-20">
        <img className="w-[15vw] sm:w-[8vw]" src="assets/insta.png" alt="Instagram" />
      </div>
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center text-center">
        <h1 className="cool-text text-[3rem] sm:text-7xl md:text-[5.5rem] font-[800] leading-[1.05] tracking-tight text-[#1a1a1a]">
          Everything{" "}
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-black via-[#9186BB] to-[#9186BB] font-black italic pr-8">
            Trending
          </span>
          <br />
          Across{" "}
            ...
            <div className="relative bg-white/50 rounded-[2rem] overflow-hidden min-h-[220px] flex flex-col">
              <div className="relative flex-grow">
                {showTyping && (
                  <span
                    ref={placeholderRef}
                    className="absolute left-8 top-8 text-slate-400 pointer-events-none text-lg font-medium opacity-60 italic"
                  />
                )}
                <textarea
                  ref={inputRef}
